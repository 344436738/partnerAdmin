$(function() {
	...
	$('#table').bootstrapTable( {
		...
		url : 'weixinNewstemplate/dataList.do',
		idField : "id",
		...
		} ,
		{
			...
			formatter : function(value, row, index) {
				var html = "<a href='javascript:void(0);' onclick='addPic("+row.id+")'>添加图文</a>&nbsp&nbsp&nbsp&nbsp<a href='javascript:void(0);' onclick='queryPic("+row.id+")'>查看图文</a>";
				return html;
			}
		}]
	});
...
function addPic(id){
	layerPop=layer.open({
	    type: 2, 
	    area: ['1000px', '600px'],
	    content:['weixinNewsitem.do?goAdd&templateId='+id]
	}); 
	
}
//查看图文
function queryPic(id){
	layerPop=layer.open({
	    type: 2, 
	    area: ['1300px', '600px'],
	    content:['weixinNewstemplate.do?goMessage&templateId='+id]
	}); 
}
...
function del() {
	...
	//不显示遮罩
			...
			$.post("weixinNewstemplate/delete.do", data, function(result) {
...
function loadData(id) {
	...
	//loadPage("${msUrl}/weixinNewstemplate/getId.do");
	$.post("weixinNewstemplate/getId.do",{id:id},function(result){
		if(result.success){
