...
function queryParams(params) {
	return {
		...
		fatherid : $("#_querypid").val()
	};
}
//查询子菜单
function toList(fatherid){
	$("#_querypid").val(fatherid);
...
function del() {
	...
	//不显示遮罩
			...
			$.post("weixinMenu/delete.do", data, function(result) {
	...
	/* $("#editDiv").removeClass("hidden");
		layerPop=layer.open({
		    ...
		    success: function(layero, index){
		    	var fatherid =$('#_querypid').val();
				if(fatherid){
					$("#edit_parentId").val(fatherid);
				}
...
function loadData(id) {
	...
	//loadPage("${msUrl}/weixinMenu/getId.do");
	$.post("weixinMenu/getId.do",{id:id},function(result){
		if(result.success){
