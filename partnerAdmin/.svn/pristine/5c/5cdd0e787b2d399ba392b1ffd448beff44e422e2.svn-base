...
function handle(){
	var array=$("#scoreexchangetable").bootstrapTable('getSelections');
	
	if(array.length!=1){
		layer.msg('请选择单条记录', {tips:2});
		return;
	}
	
	$.post("scoreExchangeHandle.do", {id:array[0].id,paystatus:array[0].paystatus,exchangescore:array[0].exchangescore,userid:array[0].userid}, 
			function(result) {
		if (result.success) {
			$('#scoreexchangetable').bootstrapTable('refresh');
...
function checknotpass(){
	...
	$.post("resumeCheckNot.do", JSON.stringify(allArray), function(result) {
		if (result.success) {
			$('#cooperationtable').bootstrapTable('refresh');
			layer.msg(result.msg, {
				tips : 2
			});
		} else {
