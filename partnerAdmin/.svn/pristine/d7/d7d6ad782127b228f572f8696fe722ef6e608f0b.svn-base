$(function() {
	...
	$('#table').bootstrapTable( {
		...
		url : 'weixinAttention/dataList.do',
		idField : "id",
...
function sync() {
	...
	$.post("weixinAttention/sync.do",function(result){
		if(result.success){
			 
			layer.close(loading);
			
		}else{
			layer.msg('同步失败', {tips:2});
		}
	},"json");
}
//添加
...
function loadData(id) {
	var loading=layer.load(2);
	//loadPage("${msUrl}/weixinAttention/getId.do");
	$.post("weixinAttention/getId.do",{id:id},function(result){
		if(result.success){
