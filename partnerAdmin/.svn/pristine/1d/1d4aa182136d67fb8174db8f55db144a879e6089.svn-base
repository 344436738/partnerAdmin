...
//			System.out.println(xPath);
	...
	public int getQueryNode(String filePath, String xPath, String newNode,
			Map<String, String> attrMap, String text) throws Exception {
		/**
		 * 用XPath.selectSingleNode(root,xpath)方法来查询具体某个节点�?? 根据单个属�?�查询： Element
		 * findelement
		 * =(Element)XPath.selectSingleNode(root,"/root/biaodan[@bdname='"
		 * +chkname+"']"); 根据多个属�?�查询： Element
		 * findelement=(Element)XPath.selectSingleNode
		 * (root,"/root/biaodan[@bdname='"
		 * +chkname+"'][@liuname='"+flowname+"']"); 根据多个属�?�查询，（多属�?�之间或的关系）�?
		 * Element findelement=(Element)XPath.selectSingleNode(root,
		 * "/root/biaodan[@bdname='"
		 * +chkname+"'] | /root/biaodan[@liuname='"+flowname+"']");
